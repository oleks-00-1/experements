...
# --- Налаштування ---
INPUT = "video.mp4"          # файл або папка нижче
OUTPUT_FOLDER = "audio_out"  # папка для збереження аудіо
MP3_BITRATE = "192k"         # бітрейт для перекодування в MP3

# --- Створюємо папку для аудіо, якщо не існує ---
os.makedirs(OUTPUT_FOLDER, exist_ok=True)

# --- Функція для визначення кодеку аудіо ---
...
def extract_audio(video_path, output_folder, mp3_bitrate="192k"):
    ...
    # Якщо кодек вже сумісний з MP3, перекодуємо
    ...
    # Якщо кодек не MP3, але ми можемо просто копіювати без втрат
        ...
        # Інші кодеки — перекодовуємо в MP3
        output_audio = os.path.join(output_folder, f"{base_name}.mp3")
        cmd = [
            "ffmpeg", "-i", video_path, "-vn", "-ab", mp3_bitrate,
            "-y", output_audio
...
# --- Для всієї папки з MP4 (розкоментувати, якщо потрібно) ---
